    ...
    try {
      if (userExist) {
        ...
        sendEmail({
          ...
          from: process.env.GOOGLE_EMAIL || "fetnupdates@gmail.com",
          to: userEmail,
        ...
        res
          .status(201)
          .json({
            message: `An Email has been sent to your account : ${userEmail} please verify`,
          });
      }
    } catch (error) {
      res.status(500).send(error.message);
...
module.exports = router.post("/signInUser", async (req, res) => {
  ...
  try {
    if (userExist && matchPassword) {
      ...
      res
        .status(400)
        .json({ message: "invalid crededntials or unverified account" });
    }
  } catch (error) {
    res.status(500).send(error.message);
    console.log(error);
